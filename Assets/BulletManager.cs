...
public class BulletManager : MonoBehaviour {
	...
	public static void SpawnBullet(Vector2 startPosition, Quaternion startRotation, int pathType, float speed, float intensity) {
	...
	public static void SpawnInRectArea(Vector2 bottomLeft, Vector2 topRight, Quaternion startRotation, int pathType, float speed, float intensity) {
		float xPos = Random.Range(bottomLeft.x, topRight.x);
		float yPos = Random.Range(bottomLeft.y, topRight.y);
		
		SpawnBullet(new Vector2(xPos, yPos), startRotation, pathType, speed, intensity);
	}

	public static void SpawnBurst(Vector2 center, int quantity, Quaternion startRotation, int pathType, float speed, float intensity) {
		for (int i = 0; i < quantity; i++) {
			SpawnBullet(center, startRotation * Quaternion.Euler(0f, 0f, i*360f/quantity), pathType, speed, intensity);
		}
	}
}
